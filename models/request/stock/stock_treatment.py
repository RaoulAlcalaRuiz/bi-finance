...
class StockTreatment:
    ...
    def get_goal(self):
        annual_goal = []
        for i in range(1, 13):
            month_result = self.month_stock_goal(i)
    ...
    def get_result(self):
        annual_goal = []
        for i in range(1, 13):
            month_result = self.month_stock_result(i)
            annual_goal.append(month_result)
        return annual_goal

    # Month request
    def month_stock_goal(self, month):
        self.change_month(month)
        result_sql = self._sql.get_goal_stock_value()
        return self.interpretation(result_sql)

    def month_stock_result(self, month):
        self.change_month(month)
        result_sql = self._sql.get_stock_value()
        if len(result_sql) != 0:

...
from ..classes.date_delivery import DateDelivery, compute_achievement, compute_achievement_list, _value_null_in_futur
...
class ReportGoalEmployee(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # ux data
        goal_achieved = compute_achievement_list(list_in_time, percent_wanted)

        # Qualité
        quality = self.get_quality_each_month(year)
    ...
    def get_quality_each_month(self, year):
        new_list_formatted = []
        new_list = []
        good_quality_sum = 0
        all_quality_sum = 0
        for month in range(1, 13):
            ...
            good_quality = self.get_quality_delivery(year, month, "False")
            all_quality = self.get_quality_delivery(year, month, "True") + good_quality

            good_quality_sum += good_quality
            all_quality_sum += all_quality

            average_quality = self._average_delivery(good_quality, all_quality)
            average_quality_sum = self._average_delivery(good_quality_sum, all_quality_sum)
            goal_quality = self.get_quality_goal(year, month)


            new_list_formatted.append((month,
                                       [_value_null_in_futur(self._format_percent(average_quality), year, today_year, month, today_month),
                                        compute_achievement(average_quality, goal_quality)
                                        ],
                                       _value_null_in_futur(self._format_percent(average_quality_sum), year, today_year, month, today_month),
                                       self._format_percent(goal_quality),
                                       _value_null_in_futur(all_quality - good_quality, year, today_year, month, today_month)
                                       ))

            new_list.append((month,
                             _value_null_in_futur(average_quality, year, today_year, month, today_month),
                             _value_null_in_futur(average_quality_sum, year, today_year, month, today_month),
                             goal_quality,
                             _value_null_in_futur(all_quality - good_quality, year, today_year, month, today_month)
                             ))

        return [new_list_formatted, new_list]

    def get_quality_delivery(self, year, month, bad_quality):
        month_formatted = "{:02d}".format(month)
        request = ("select COUNT(s.problematic_quality) as Good " +
                   "From sale_order s  " +
                   "WHERE TO_CHAR(s.commitment_date, 'YYYY MM') = '" + str(year) + " " + str(month_formatted) + "' " +
